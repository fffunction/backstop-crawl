...
const defaultConf = fs.existsSync(confPath) ? JSON.parse(fs.readFileSync(confPath)) : require('./backstop.template.json');
...
function crawl(url, flags) {
    ...
    crawler.on('fetchcomplete', queueItem => {
        if (queueItem.stateData.contentType.indexOf('text/html') > -1) {
            ...
            const currentScenario = JSON.parse(JSON.stringify(defaultScenario));
            currentScenario.label = queueItem.path;
            if( flags.referenceUrl ){
                currentScenario.referenceUrl = queueItem.url.replace(url, flags.referenceUrl);
            } else if( Object.prototype.hasOwnProperty.call(currentScenario, "referenceUrl") ){
