...
const defaultConf = fs.existsSync(confPath) ? JSON.parse(fs.readFileSync(confPath)) : JSON.parse(fs.readFileSync('./backstop.template.json'));
...
function crawl(url, flags) {
    ...
    crawler.on('fetchcomplete', queueItem => {
        if (queueItem.stateData.contentType.indexOf('text/html') > -1) {
            ...
            const currentScenario = {...defaultScenario};
